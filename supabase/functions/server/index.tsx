...
// Delete user account
app.delete("/make-server-026f502c/users/:userId", async (c) => {
  try {
    ...
    // 2. Delete events created by user
    await supabase.from('events').delete().eq('host_id', userId);

    // 3. Delete follows (both following and followers)
    ...
    // 5. Delete place comments
    await supabase.from('place_comments').delete().eq('user_id', userId);
    ...
    if (deleteProfileError) {
      ...
      return c.json({ error: deleteProfileError.message }, 500);
    }
    ...
    if (deleteAuthError) {
      console.warn('Warning: Could not delete auth user (profile already deleted):', deleteAuthError);
      // We still return success because the profile is deleted, which is the important part
    }
...
app.get("/make-server-026f502c/events", async (c) => {
  try {
    ...
    // Log which user IDs are missing (likely deleted accounts)
    const foundUserIds = rsvpUsersData?.map((u: any) => u.id) || [];
    const missingUserIds = rsvpUserIds.filter(id => !foundUserIds.includes(id));
    if (missingUserIds.length > 0) {
      console.warn('⚠️ [SERVER] Some RSVP users not found (likely deleted):', missingUserIds.length, 'users');
    }

    // Attach user data to RSVPs - filter out RSVPs from deleted users
    const rsvpsData = (rsvpsDataRaw || [])
      .map((rsvp: any) => {
      ...
      .filter((rsvp: any) => rsvp.user !== null); // Remove RSVPs from deleted users
  ...
  try {
    ...
    const commentsData = (commentsDataRaw || []).map((comment: any) => {
      ...
      return {
        ...
        text: comment.content || comment.text, // Support both 'content' and 'text' columns
...
app.post("/make-server-026f502c/events", async (c) => {
  ...
  try {
    ...
    const body = await c.req.json();
    console.log(`⏱️ [CREATE EVENT] Parsed body in ${Date.now() - startTime}ms`);
    ...
    console.log(`📋 [CREATE EVENT] Creating event for ethnicity: ${ethnicityId}`);
    ...
    console.log(`⏱️ [CREATE EVENT] About to insert event (${Date.now() - startTime}ms elapsed)`);
    ...
    console.log(`⏱️ [CREATE EVENT] Insert completed in ${Date.now() - startTime}ms`);

    if (eventError) {
      console.error('❌ [CREATE EVENT] Database error:', eventError);
      return c.json({ error: 'Failed to create event', details: eventError }, 500);
    }

    console.log(`✅ [CREATE EVENT] Event created successfully in ${Date.now() - startTime}ms - ID: ${eventData.id}`);
    return c.json({ event: eventData });
  } catch (error) {
    console.error(`❌ [CREATE EVENT] Unexpected error after ${Date.now() - startTime}ms:`, error);
    return c.json({ error: 'Internal server error', details: String(error) }, 500);
  }
});
...
// Add event comment (bypasses RLS - anyone logged in can comment)
app.post("/make-server-026f502c/events/:eventId/comments", async (c) => {
  try {
    const eventId = c.req.param('eventId');
    const body = await c.req.json();
    ...
    console.log('💬 Adding comment to event:', eventId, 'by user:', userId);

    // Insert comment (bypasses RLS)
    const { data: commentData, error: commentError } = await supabase
      .from('comments')
      .insert({
        event_id: eventId,
        user_id: userId,
        content: text, // Using 'content' column instead of 'text'
      })
      .select()
      .single();
    ...
    if (commentError) {
      ...
      return c.json({ error: 'Failed to add comment', details: commentError }, 500);
    }
    ...
    const comment = {
      ...
      text: commentData.content, // Map 'content' column to 'text' field
...
// Get event promotions (bypasses RLS - for admin)
app.get("/make-server-026f502c/event-promotions", async (c) => {
  try {
    ...
    if (error) {
      console.error('❌ [SERVER] Get promotions error:', error);
    ...
    console.error('❌ [SERVER] Unexpected error getting promotions:', error);
    return c.json({ error: 'Internal server error' }, 500);
  }
});

// Create event promotion (bypasses RLS)
app.post("/make-server-026f502c/event-promotions", async (c) => {
  const requestTime = new Date().toISOString();
  console.log(`📊 [SERVER ${requestTime}] NEW PROMOTION REQUEST RECEIVED`);

  try {
    const body = await c.req.json();
    console.log(`📊 [SERVER ${requestTime}] Raw request body:`, JSON.stringify(body));

    const { eventId, userId, package: packageType, price } = body;

    if (!eventId || !userId || !packageType || !price) {
      console.error(`❌ [SERVER ${requestTime}] Missing required fields:`, { eventId, userId, packageType, price });
      return c.json({ error: 'Missing required fields' }, 400);
    }

    console.log(`📊 [SERVER ${requestTime}] Creating event promotion:`, {
      event_id: eventId,
      user_id: userId,
      package: packageType,
    ...
    if (promotionError) {
      console.error(`❌ [SERVER ${requestTime}] Promotion creation error:`, promotionError);
      return c.json({ error: 'Failed to create promotion request', details: promotionError }, 500);
    }

    console.log(`✅ [SERVER ${requestTime}] Promotion created successfully:`, promotionData.id);
    return c.json({ promotion: promotionData });
  } catch (error) {
    console.error(`❌ [SERVER ${requestTime}] Unexpected error creating promotion:`, error);
    return c.json({ error: 'Internal server error' }, 500);
  }
});
...
app.post("/make-server-026f502c/ad-requests", async (c) => {
  try {
    ...
    console.log('✅ Ad request submitted:', requestId, 'Payment Intent:', paymentIntentId);
...
app.get("/make-server-026f502c/ad-requests", async (c) => {
  try {
    ...
    console.log('📋 [GET AD REQUESTS] Fetching ad requests with status:', status || 'all');

    // Get all ad requests from KV store (getByPrefix already returns values)
    let requests = await kv.getByPrefix('ad-request:');

    console.log('📋 [GET AD REQUESTS] Found', requests?.length || 0, 'total requests');

    // Filter by status if provided
    if (status) {
      requests = requests.filter((req: any) => req?.status === status);
      console.log('📋 [GET AD REQUESTS] After status filter:', requests.length, 'requests');
    }
    ...
    console.log('📋 [GET AD REQUESTS] Returning', requests.length, 'requests');
...
// Create payment intent for ad request (authorization only)
app.post("/make-server-026f502c/ad-requests/create-payment-intent", async (c) => {
  try {
    console.log('💳 [CREATE PAYMENT INTENT] Starting...');

    if (!stripe) {
      console.error('❌ [CREATE PAYMENT INTENT] Stripe not initialized - missing STRIPE_SECRET_KEY');
      return c.json({ error: "Payment system not configured. Please contact support." }, 500);
    }

    const body = await c.req.json();
    const { amount, currency, customerEmail, description, metadata } = body;

    console.log('📋 [CREATE PAYMENT INTENT] Request:', { amount, currency, customerEmail, description });
    ...
    const paymentIntent = await stripe.createPaymentIntent({
      ...
      customerEmail,
      description: description || 'Ad Request Payment',
      metadata,
    });

    console.log('✅ [CREATE PAYMENT INTENT] Success:', paymentIntent.id);
    ...
    console.error('❌ [CREATE PAYMENT INTENT] Error details:', error.message, error.stack);
    return c.json({
...
app.patch("/make-server-026f502c/ad-requests/:requestId", async (c) => {
  try {
    ...
    // If approving and there's a payment intent, capture it
    if (status === 'approved' && existing.paymentIntentId) {
      try {
        console.log('💰 Capturing payment for approved ad request:', requestId);
        await stripe.capturePaymentIntent(existing.paymentIntentId);
        ...
        return c.json({
          ...
          details: String(captureError)
        }, 500);
      }
    }

    // If rejecting and there's a payment intent, cancel it
    if (status === 'rejected' && existing.paymentIntentId) {
      try {
        console.log('❌ Canceling payment for rejected ad request:', requestId);
        await stripe.cancelPaymentIntent(existing.paymentIntentId);
        console.log('✅ Payment authorization canceled');
      } catch (cancelError) {
        console.error('⚠️ Failed to cancel payment (continuing anyway):', cancelError);
        // Don't fail the whole request if cancel fails
      }
    }
    ...
    await kv.set(requestKey, {
      ...
      updatedAt: new Date().toISOString(),
      paymentCapturedAt: status === 'approved' && existing.paymentIntentId ? new Date().toISOString() : existing.paymentCapturedAt,
    });
