...
import { ArrowLeft, Globe2, Check, LogOut, Bell, Megaphone, HelpCircle, AlertTriangle, Trash2, Plus, X } from 'lucide-react';
...
export function SettingsPage() {
  ...
  const [notificationCities, setNotificationCities] = useState<string[]>(user?.notificationCities || []);
  ...
  const handleAddCity = () => {
    ...
    if (trimmedCity && !notificationCities.includes(trimmedCity)) {
      setNotificationCities([...notificationCities, trimmedCity]);
      setNewCity('');
      toast.success(`Added ${trimmedCity}`);
    } else if (notificationCities.includes(trimmedCity)) {
      toast.error('City already added');
    }
  };

  const handleRemoveCity = (cityToRemove: string) => {
    setNotificationCities(notificationCities.filter(city => city !== cityToRemove));
    toast.success(`Removed ${cityToRemove}`);
  };

  const handleSave = () => {
    // Save notification preferences if they've changed
    const citiesChanged = JSON.stringify(notificationCities) !== JSON.stringify(user?.notificationCities || []);
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="notificationCities">Notification Cities</Label>
              <p className="text-xs text-gray-500 mt-1 mb-3">Track events in multiple cities</p>

              {/* City input with add button */}
              ...
              {notificationCities.length === 0 && (
                <p className="text-xs text-gray-400 mt-2 italic">No cities added yet. Add cities to receive event notifications.</p>
              )}
            </div>
