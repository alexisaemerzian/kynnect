    ...
    try {
      ...
      if (event) {
        ...
        if (followers.length > 0) {
          console.log(`📢 Checking notification preferences for ${followers.length} followers...`);
          
          // Filter followers based on notification preferences
          const { followerIds: notifiableFollowers } = await getNotifiableFollowers(userId, followers);
          
          console.log(`📢 Notifying ${notifiableFollowers.length} of ${followers.length} followers...`);
          
          if (notifiableFollowers.length > 0) {
            const { success } = await notifyFollowersOfNewEvent(userId, event.id, eventTitle, notifiableFollowers);
            
            if (success) {
              toast.success(`Event created! ${notifiableFollowers.length} follower${notifiableFollowers.length !== 1 ? 's' : ''} notified.`);
            } else {
