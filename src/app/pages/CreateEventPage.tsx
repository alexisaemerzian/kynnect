...
export function CreateEventPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Safety timeout - reset button if something hangs
    const safetyTimeoutId = setTimeout(() => {
      console.error('❌ Event creation timed out - resetting button');
      toast.error('Request timed out. Please try again. If this persists, try without an image first.');
      setIsLoading(false);
    ...
    try {
      ...
      try {
        ...
        console.log('✅ Geocoding complete:', coordinates);
      } catch (geocodeError) {
        console.warn('⚠️ Geocoding failed (non-critical):', geocodeError);
      ...
      // Ensure timeout is cleared before success actions
      clearTimeout(safetyTimeoutId);
