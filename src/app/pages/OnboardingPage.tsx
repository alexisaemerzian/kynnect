...
export function OnboardingPage() {
  ...
  // Get colors from first selected ethnicity for dynamic background
  const selectedEthnicity = selectedIds.length > 0
    ? ethnicities.find(e => e.id === selectedIds[0])
    : null;
  ...
  const toggleEthnicity = (id: string) => {
    setSelectedIds(prev => {
      if (prev.includes(id)) {
        ...
        // Limit to maximum 3 ethnicities
        if (prev.length >= 3) {
          return prev;
        }
        return [...prev, id];
  ...
  // Create dynamic gradient background based on selected ethnicity
  const backgroundStyle = selectedEthnicity
    ? {
        background: `linear-gradient(135deg, ${selectedEthnicity.primaryColor}40 0%, ${selectedEthnicity.secondaryColor}50 50%, ${selectedEthnicity.primaryColor}60 100%)`
      }
    : {};

  return (
    <div
      className="min-h-screen bg-gradient-to-br from-purple-50 via-orange-50 to-pink-50 flex items-center justify-center p-4 transition-all duration-700"
    ...
    >
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold mb-3">Welcome to Kynnect</h1>
          <p className="text-2xl font-semibold bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-2">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6 max-h-[500px] overflow-y-auto">
          <p className="text-sm text-gray-600 mb-4">
            Select up to 3 communities • {selectedIds.length} of 3 selected
          </p>
        ...
        <p className="text-center text-xs text-gray-400 mt-2">
          © 2026 Kynnect™. All rights reserved.
        </p>
      </div>
    </div>
  );
