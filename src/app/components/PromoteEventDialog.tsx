...
export function PromoteEventDialog({
  ...
  const promotionPackages = useMemo(() => [
    ...
    {
      id: 'standard' as PromotionPackage,
      name: '7-Day Featured',
      duration: 7,
      price: 35,
      features: [
    ...
    {
      id: 'premium' as PromotionPackage,
      name: '14-Day Premium',
      duration: 14,
      price: 60,
      features: [
  ...
  const [selectedPackage, setSelectedPackage] = useState<PromotionPackage>('standard');
  ...
  const handleSubmit = async () => {
    ...
    const validPackages: PromotionPackage[] = ['basic', 'standard', 'premium'];

    if (!validPackages.includes(selectedPackage)) {
      console.error('❌ Invalid package:', selectedPackage);
      toast.error('Invalid promotion package selected');
      return;
    }

    console.log(`🚀 [PromoteEvent ${requestId}] NEW REQUEST AT ${new Date().toISOString()}`);
    console.log(`🚀 [PromoteEvent ${requestId}] Package:`, selectedPackage);

    setIsSubmitting(true);

    try {
      const { promotion, error } = await createEventPromotion(
        { eventId, package: selectedPackage, price: selectedPkg.price },
        user.id
      );

      console.log(`🚀 [PromoteEvent ${requestId}] Result:`, { promotion, error });

      if (error) {
        console.error(`❌ [PromoteEvent ${requestId}] Error:`, error);
        toast.error(`Error: ${error}`);
        return;
      }

      if (promotion) {
        console.log(`✅ [PromoteEvent ${requestId}] Success!`);
        toast.success('Promotion request submitted! We\'ll review it shortly.');
        onOpenChange(false);
      }
    } catch (err) {
      console.error(`❌ [PromoteEvent ${requestId}] Unexpected error:`, err);
