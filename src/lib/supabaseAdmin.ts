...
export async function createEventPromotion(
  ...
  try {
    console.log('📊 [createEventPromotion] Starting insert...', {
      event_id: promotionData.eventId,
      user_id: userId,
    ...
    console.log('📊 [createEventPromotion] Insert result:', { data, error });

    if (error) {
      console.error('❌ [createEventPromotion] Database error:', error);
      return { promotion: null, error: 'Failed to create promotion request' };
    }
    
    console.log('✅ [createEventPromotion] Success!');
    return { promotion: data as unknown as EventPromotion, error: null };
  } catch (error) {
