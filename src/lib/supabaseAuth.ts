...
export async function signIn(email: string, password: string) {
  try {
    ...
    // Load available ethnicities from profile (user can have multiple ethnicities in ONE account)
    if (profile.available_ethnicities && Array.isArray(profile.available_ethnicities)) {
      localStorage.setItem('availableEthnicities', JSON.stringify(profile.available_ethnicities));
      console.log('🌍 User has access to ethnicities:', profile.available_ethnicities);
    } else if (profile.ethnicity_id) {
      // Fallback: if available_ethnicities is not set, use the current ethnicity_id
      localStorage.setItem('availableEthnicities', JSON.stringify([profile.ethnicity_id]));
      console.log('🌍 User has access to ethnicity:', profile.ethnicity_id);
    }
  ...
  try {
    ...
    if (!profile) {
      ...
      try {
        ...
        // Load available ethnicities from profile
        if (newProfile.available_ethnicities && Array.isArray(newProfile.available_ethnicities)) {
          localStorage.setItem('availableEthnicities', JSON.stringify(newProfile.available_ethnicities));
          console.log('🌍 User has access to ethnicities:', newProfile.available_ethnicities);
        } else if (newProfile.ethnicity_id) {
          localStorage.setItem('availableEthnicities', JSON.stringify([newProfile.ethnicity_id]));
          console.log('🌍 User has access to ethnicity:', newProfile.ethnicity_id);
        }
...
export async function updateUserProfile(userId: string, updates: Partial<UserData>) {
  try {
    ...
    const updateData = {
      ...
      notification_cities: updates.notificationCities ? JSON.stringify(updates.notificationCities) : undefined,
...
export async function deleteAccount(userId: string) {
  try {
    console.log('🗑️ Deleting account via server:', userId);

    // Use server endpoint to delete account (requires service role for auth deletion)
...
export function dbUserToUserData(dbUser: any): UserData {
  // Parse notification cities from JSON if available
  let notificationCities: string[] | undefined;
  if (dbUser.notification_cities) {
    try {
      notificationCities = typeof dbUser.notification_cities === 'string'
        ? JSON.parse(dbUser.notification_cities)
        : dbUser.notification_cities;
    } catch (e) {
      console.error('Failed to parse notification_cities:', e);
      notificationCities = [];
    }
  }
