-- ============================================
-- KYNNECT SUPABASE DATABASE SETUP (FIXED)
...
CREATE INDEX IF NOT EXISTS idx_users_ethnicity ON users(ethnicity_id);
...
CREATE INDEX IF NOT EXISTS idx_places_submitted_by ON places(submitted_by_id);
...
CREATE INDEX IF NOT EXISTS idx_promotions_event ON event_promotions(event_id);
CREATE INDEX IF NOT EXISTS idx_promotions_user ON event_promotions(user_id);
CREATE INDEX IF NOT EXISTS idx_promotions_status ON event_promotions(status);
CREATE INDEX IF NOT EXISTS idx_promotions_dates ON event_promotions(start_date, end_date);
...
BEGIN
  ...
  DROP POLICY IF EXISTS "Event hosts can update RSVPs" ON rsvps;
  ...
  -- Places policies
  DROP POLICY IF EXISTS "Anyone can view approved places" ON places;
  DROP POLICY IF EXISTS "Authenticated users can submit places" ON places;
  DROP POLICY IF EXISTS "Users can update own pending places" ON places;
  ...
  -- Event promotions policies
  DROP POLICY IF EXISTS "Users can view own promotions" ON event_promotions;
  DROP POLICY IF EXISTS "Admins can view all promotions" ON event_promotions;
  DROP POLICY IF EXISTS "Authenticated users can create promotions" ON event_promotions;
  DROP POLICY IF EXISTS "Admins can update promotions" ON event_promotions;
END $$;
